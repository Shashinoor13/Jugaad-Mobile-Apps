...
class PhonePeServices {
  ...
  Future<void> handlePayClick(BuildContext context,
    ...
    if (isTypeCard) {
      ...
      // Fixed: Use correct method to get installed UPI apps for Android
      String? upiAppListResponse =
          await PhonePePaymentSdk.getUpiAppsForAndroid();
      if (upiAppListResponse == null) {
        toast('Unable to fetch installed UPI apps.');
        return;
      }
...
Future<void> getPackageSignatureForAndroid() async {
  if (Platform.isAndroid) {
    try {
      final packageSignature = await PhonePePaymentSdk;
      print('PhonePeSdk packageSignature $packageSignature');
    } catch (error) {
      print('Error getting package signature: $error');
    }
  }
}
