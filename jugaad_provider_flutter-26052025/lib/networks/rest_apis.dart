...
import 'package:handyman_provider_flutter/models/category_response.dart';
...
Future<void> clearPreferences() async {
  ...
  await appStore.setReferralCode('');
  await appStore.setLastName('');
  if (!getBoolAsync(IS_REMEMBERED)) await appStore.setUserEmail('');
  if (!getBoolAsync(IS_REMEMBERED)) await appStore.setUserName('');
  ...
  await appStore.setVerifiedPlanStatus(false);
...
Future<LoginResponse> loginUser(Map request) async {
  LoginResponse res = LoginResponse.fromJson(await (handleResponse(await buildHttpResponse('provider-login', request: request, method: HttpMethodType.POST))));

  return res;
}

Future<void> saveUserData(UserData data) async {
  // if (data.status == 1) { //ACME
    // if (data.apiToken != null) await appStore.setToken(data.apiToken.validate());

    await appStore.setUserId(data.id.validate());
    await appStore.setUserStatus(data.status.validate());
    await appStore.setFirstName(data.firstName.validate());
    await appStore.setReferralCode(data.referralCode.validate());
    ...
    if (data.subscription != null) {
      await setSaveSubscription(
        ...
        isVerifiedPlan: data.subscription!.isVerifiedPlan.validate(),
      );
    }else{
      await setSaveSubscription(
        isSubscribe: 0,
...
Future<List<CityListResponse>> getAdminCityList() async {
  Iterable res = await (handleResponse(await buildHttpResponse('city-list-get', method: HttpMethodType.GET)));
  return res.map((e) => CityListResponse.fromJson(e)).toList();
}
//endregion
...
Future<List<CategoryFilterData>> getCategoryFilter({required List<CategoryFilterData> list}) async {
  var res = CategoryFilterResponse.fromJson(await handleResponse(await buildHttpResponse('category-list?per_page=all', method: HttpMethodType.GET)));
  list = res.data!.cast<CategoryFilterData>();
  cachedCategoryDropdown = list;
  return list;
}

// Future<List<CategoryFilterData>> getCategoryListAll({required List<CategoryFilterData> list}) async {
//   var res = CategoryResponse.fromJson(await handleResponse(await buildHttpResponse('category-list?per_page=all', method: HttpMethodType.GET)));
//   list = res.data.cast<CategoryFilterData>();
//   cachedCategoryDropdown = list;
//   return list;
// }
...
Future<ProviderSubscriptionModel> getSubscriptionDetails() async {
  try {
    ProviderSubscriptionModel res = ProviderSubscriptionModel.fromJson(await handleResponse(await buildHttpResponse('active-subscription', method: HttpMethodType.GET)));
    appStore.setLoading(false);
    return res;
  } catch (e) {
    appStore.setLoading(false);
...
Future<void> savePayment({
  ...
  if (data != null) {
    int? iAmount = data.amount;
    if(data.isVerifiedPlan!) {
      iAmount = (iAmount! + int.parse(data.plansVerifiedProviders!.verifiedProviderAmount!));
    }
    PlanRequestModel planRequestModel = PlanRequestModel()
      ..amount = iAmount
      ..isVerifiedPlan = data.isVerifiedPlan
      ...
      ..plansVerifiedProviders = data.plansVerifiedProviders
...
Future<List<PaymentSetting>> getPaymentGateways({bool isAdvancePayment = false, bool requireCOD = true, bool requireWallet = true}) async {
  try {
    ...
    if (requireWallet && appConfigurationStore.isEnableUserWallet) {
      res.add(PaymentSetting(title: 'Wallet', type: PAYMENT_METHOD_FROM_WALLET, status: 1));
    } else {
...
Future<List<PostJobData>> getPostJobList(int page,
    {var perPage = PER_PAGE_ITEM, required List<PostJobData> postJobList,String searchText = '',String filters = '', Function(bool)? lastPageCallback}) async {
  try {
    var res = PostJobResponse.fromJson(await handleResponse(
        await buildHttpResponse('get-post-job?per_page=$perPage&page=$page'
            '&search_text=$searchText&$filters', method: HttpMethodType.GET)));
